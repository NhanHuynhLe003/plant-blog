...
export default function SectionSecond() {
  ...
  return (
    <section className="container mx-auto md:px-20 py-10">
      ...
      <div className="postList grid md:grid-cols-2 lg:grid-cols-3 gap-14">
        {dataList.map((data, index) => {
            const {id, desc, genres, img, timePublic, title, avatar, jobAuthor, nameAuthor} = data
            return Post(id, img, genres, timePublic, title, desc, nameAuthor, jobAuthor, avatar);
        })}
      </div>
    </section>
  );
...
function Post(id, imgProp, genres, timePublic, title, desc, nameAuthor, jobAuthor, avatar) {
  return (
    <div className="item">
      ...
      <div className="category flex gap-2">
        ...
        <div className="content pt-2">
          <Link href="/">
            <h1 className="title text-xl font-bold text-gray-800 hover:text-gray-600">
              {title}
            </h1>
          </Link>
